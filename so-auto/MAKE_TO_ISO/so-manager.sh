...
###############################################################################
# This script MUST be run with sudo: sudo ./so-manager.sh
###############################################################################

if [[ $EUID -ne 0 ]]; then
   echo "This script must be run as root. Use: sudo ./so-manager.sh"
   exit 1
fi
...
CUSTOM_WHIPTAIL="$SCRIPT_DIR/so-whiptail.manager"
...
if [[ ! -f "$CUSTOM_WHIPTAIL" ]]; then
  echo "ERROR: custom so-whiptail.manager not found in:"
  echo "  $SCRIPT_DIR"
  exit 1
fi
...
echo "Launching Security Onion installer (ISO mode)..."
